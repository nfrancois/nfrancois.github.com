...
$.draw = function() {
  var time = $.mul($.DateImplementation$now$0().millisecondsSinceEpoch, 0.002);
  var x = $.add($.mul($.sin(time), 96), 128);
  var y = $.add($.mul($.cos($.mul(time, 0.9)), 96), 128);
  $.ctx.set$fillStyle('rgb(245,245,245)');
  $.ctx.fillRect$4(0, 0, 255, 255);
  $.ctx.set$fillStyle('rgb(255,0,0)');
  $.ctx.beginPath$0();
  $.ctx.arc$6(x, y, 10, 0, 6.283185307179586, true);
  $.ctx.closePath$0();
  $.ctx.fill$0();
};
...
$.main = function() {
  $.ctx = $.document().query$1('#canvas').getContext$1('2d');
  $.window().requestAnimationFrame$1($.animate);
};
